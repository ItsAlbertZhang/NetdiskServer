...
在客户端运行后初次连接服务端时(即进行 REQCONF 操作时), 服务端会将 `服务端的连接文件描述符`, `初次连接时间` 与 `客户端 ip 地址` 使用 **公钥** 进行加密, 并将密文发送给客户端. 这段密文即作为 token 使用.

客户端建立新连接时, 只需将 token 发送给服务端. 服务端在接收到 token 后, 使用私钥进行解密, 随后在 **现有连接** 中寻找连接文件描述符与 ip 地址均相符的连接, 或在 **休眠连接** 中寻找初次连接时间与 ip 地址均相符的连接, 即可验证客户端身份.
